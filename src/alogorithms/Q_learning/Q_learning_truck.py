...
from ...plot_figure.plot_route_truck import plot_route_truck_q,plot_route_drone_q
drone_speed=43.2
truck_speed=50
drone_capacity=1
maxDisDrone=4
...
class Q_learning():
    ...
    def update_q_table(self,state, action, reward, next_state, next_action, counting_state):
        ...
        if counting_state <= len(self.actions):
            q_value_real = reward + self.gamma * self.q_table.loc[next_state, :].max()
        else:
            q_value_real = reward
...
def run():
    mainn=Q_learning(action=indexes)
    mainn.train()
    return mainn.route
    # print(mainn.route)
...
def Q_Learning():
    main=Q_learningD(route=route,action=indexes)
    main.train()
    with open("D:\\Tran Hoang Vu\\Lab\\VRDP\\Large Nearest Search (LNS)\\q_and_sarsa\\travelling-salesman-problem-tsp\\result\\raw\\result_q_learning.txt","w") as fio:
    ...
    plot_route_truck_q(data,main.route_truck)
    plot_route_drone_q(data,main.route_drone)