...
def run():
    mainn=Qlearning_T(action=indexes)
    return mainn.train()
...
main = Qlearning_D(action=actionForTrainingDrone)
solution = main.train()
with open("D:\\Tran Hoang Vu\\Lab\\VRDP\\Large Nearest Search (LNS)\\q_and_sarsa\\travelling-salesman-problem-tsp\\dataset\\TspD_48.txt","w") as fio:
    fio.write(f'Truck route: {solution.TruckRoute}\n')
    fio.write(f'Drone route: {solution.DroneRoute}\n')
